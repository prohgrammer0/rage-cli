...
export async function readMultilineInput(filePaths: string[] = []): Promise<InputResult> {
      ...
      for (let i = 0; i < bytes.length; i++) {
        ...
        if (b === 0x7f || b === 0x08) {
          ...
          if (cur.length > 0) {
            ...
            Deno.stdout.writeSync(enc.encode(`\x1b[A\x1b[${prev.length + 1}G\x1b[J`));
          }
          showGhost();
          continue;
        }

        // Printable ASCII: echo the raw byte — zero allocation.
        if (b >= 0x20 && b < 0x7f) {
          clearGhost();
          lines[lines.length - 1] += String.fromCharCode(b);
  ...
  return { type: "submit", text: lines.join("\n").trim() };
}
